...
export const calculateUserStats = (userMap: Map<string, UserTotal>) => {
  ...
  const stats = {
    totalExpenses: usersArray.reduce((sum, user) => sum + (user.total_amount || 0), 0),
    totalPaid: usersArray.reduce((sum, user) => sum + (user.total_paid || 0), 0),
    totalOutstanding: usersArray.reduce((sum, user) => sum + (user.remaining_balance || 0), 0)
  };
